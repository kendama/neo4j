...
json2csv.py ${projId}.json --edge_csv ${projId}_edges.csv --node_csv ${projId}_nodes.csv

sed -i -e "s/_id/_id:ID/" ${projId}_nodes.csv

head -n 1 ${projId}_nodes.csv > ${projId}_nodes_UsedURL.csv
head -n 1 ${projId}_nodes.csv > ${projId}_nodes_Activity.csv
head -n 1 ${projId}_nodes.csv > ${projId}_nodes_Entity.csv
